...
The design-agent, task-agent all read it automatically.
...
### Phase 5a — E2E Testing *(optional — requires app to be running)*

> **Skip this phase** if the application has not been implemented yet.
> The playwright-agent generates `.spec.ts` files from TEST task acceptance
...
>
> **Requirements:**
> - Application running locally at the `baseURL` configured in `playwright.config.ts`
> - Playwright installed: `npm install` (included in `package.json`)
